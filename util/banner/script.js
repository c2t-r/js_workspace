...
window.onload = function() {
    for (let n = 1; n < 6; n++) {
        ...
        ch.style.backgroundImage = `url(https://api.ambr.top/assets/UI/UI_Gacha_AvatarImg_${mapping["character"][list[n-1]][0]}.png)`;
...
function changeCharacter() {
    ...
    ch.style.backgroundImage = `url(https://api.ambr.top/assets/UI/UI_Gacha_AvatarImg_${mapping["character"][c_name][0]}.png)`;
...
function changeNamecard() {
    ...
    ch.style.backgroundImage = `url(https://api.ambr.top/assets/UI/namecard/UI_NameCardPic_${mapping["namecard"][c_name]}_P.png)`;
}
...
function tocanvas() {
    ...
    html2canvas(document.querySelector(".table_wrap"), {proxy: true, useCORS: true, onrendered: function(canvas) { canvas.toDataURL();} }).then(canvas => {
